...
def main():
    ...
    a.parameter_dir = a.parameter_dir + '_vp_hor'
    ...
    if os.path.exists(os.path.join(a.output_dir,'image_vz.npy')):
        ...
        vp_hor_init = np.load(os.path.join(a.output_dir,'vp_hor_init.npy'))
        vp_nmo_init = np.load(os.path.join(a.output_dir,'vp_nmo_init.npy'))
        ...
        vp_hor_true = np.load(os.path.join(a.output_dir,'vp_hor_true.npy'))
        vp_nmo_true = np.load(os.path.join(a.output_dir,'vp_nmo_true.npy'))
    else:
        ...
        vp_hor_init = data_set['vp_hor_init'] 
        vp_nmo_init = data_set['vp_nmo_init'] 

        vp_true = data_set['vp_true']  
        vs_true = data_set['vs_true']  
        rho_true = data_set['rho_true']  
        vp_hor_true = data_set['vp_hor_true'] 
        vp_nmo_true = data_set['vp_nmo_true'] 
        ...
        np.save(os.path.join(a.output_dir,'vp_hor_init.npy'),vp_hor_init)
        np.save(os.path.join(a.output_dir,'vp_nmo_init.npy'),vp_nmo_init)
        ...
        np.save(os.path.join(a.output_dir,'vp_hor_true.npy'),vp_hor_true)
        np.save(os.path.join(a.output_dir,'vp_nmo_true.npy'),vp_nmo_true)

    image_vz /= a.max_image
    image_vx /= a.max_image

    vp_hor_init = (vp_hor_init - a.mean_vp) / a.max_vp
    vp_hor_true = (vp_hor_true - a.mean_vp) / a.max_vp
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in range(a.max_epochs):
            ...
            for index in index_arr:
                ...
                input_vp_hor_init = vp_hor_init[[index_arr[index]],:]
                input_vp_hor_true = vp_hor_true[[index_arr[index]],:]

                input_image_vz.shape = a.nx,a.nz
                input_image_vx.shape = a.nx,a.nz
                input_vp_hor_init.shape = a.nx,a.nz
                input_vp_hor_true.shape = a.nx,a.nz



                if a.mode == 'train' or a.mode == 'export':
                    results = sess.run(fetches,
                                    feed_dict={inputs: input_image_vz, inputs2: input_vp_hor_init, inputs3: input_image_vx, targets: input_vp_hor_true})
                else:
                    zero_data = np.zeros((a.nx,a.nz))
                    results = sess.run(fetches,
                                    feed_dict={inputs: input_image_vz, inputs2: input_vp_hor_init, inputs3: input_image_vx, targets: zero_data})
                ...
                if a.figs and (a.max_epochs == 1 or (epoch + 1) % a.display_freq == 0):
                    ...
                    if index % int(len(input_image_vz)/int(len(input_image_vz))) == 0:
                        ...
                        output_res[1,:,:] = input_vp_hor_init[:,:]*a.max_vp + a.mean_vp
                        output_res[2,:,:] = input_vp_hor_true[:,:]*a.max_vp + a.mean_vp
                        ...
                        filename = os.path.join(a.output_dir, a.mode + str(index_arr[index]) + '_vp_hor.dat')
            ...
            if should(epoch, a.store_weights_freq) and a.mode =='train':
                ...
                loss_filename = os.path.join(a.output_dir, 'L2_loss_' + a.mode + '_vp_hor.dat')
