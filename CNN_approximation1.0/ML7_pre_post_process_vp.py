...
def load_train_data(input_dir, prefix, nx, nz):
    ...
    vp_hor_init = np.zeros((len(file_list),nx*nz))
    vp_nmo_init = np.zeros((len(file_list),nx*nz))
    vp_true = np.zeros((len(file_list),nx*nz))
    vs_true = np.zeros((len(file_list),nx*nz))
    rho_true = np.zeros((len(file_list),nx*nz))
    vp_hor_true = np.zeros((len(file_list),nx*nz))
    vp_nmo_true = np.zeros((len(file_list),nx*nz))
    for i,filename in enumerate(file_list):
        print(" input: " + filename)
        data_set = np.loadtxt(filename)  
        len_data = len(data_set)

        image_vz[i,:] = data_set[0*int(len_data/12):1*int(len_data/12)] #picked data
        image_vx[i,:] = data_set[1*int(len_data/12):2*int(len_data/12)] #picked data
        vp_init[i,:] = data_set[2*int(len_data/12):3*int(len_data/12)] #picked data
        vs_init[i,:] = data_set[3*int(len_data/12):4*int(len_data/12)] #picked data
        rho_init[i,:] = data_set[4*int(len_data/12):5*int(len_data/12)] #picked data
        vp_true[i,:] = data_set[5*int(len_data/12):6*int(len_data/12)] #picked data
        vs_true[i,:] = data_set[6*int(len_data/12):7*int(len_data/12)] #picked data
        rho_true[i,:] = data_set[7*int(len_data/12):8*int(len_data/12)] #picked data
        vp_hor_true[i,:] = data_set[8*int(len_data/12):9*int(len_data/12)] #picked data
        vp_nmo_true[i,:] = data_set[9*int(len_data/12):10*int(len_data/12)] #picked data
        vp_hor_init[i,:] = data_set[10*int(len_data/12):11*int(len_data/12)] #picked data
        vp_nmo_init[i,:] = data_set[11*int(len_data/12):12*int(len_data/12)] #picked data
    ...
    data['vp_hor_true'] = vp_hor_true
    data['vp_nmo_true'] = vp_nmo_true
    data['vp_hor_init'] = vp_hor_init
    data['vp_nmo_init'] = vp_nmo_init
    return data
