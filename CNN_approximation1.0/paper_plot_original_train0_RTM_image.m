...
%     input_data = dlmread([num_shot 'shots' num_model 'samples' num_iter 'iterations_FWIuse_1st_iteration_FWI_model/real_outputs_using_vp_mig/real0.dat']);
    ...
    caxis([-cvalue cvalue]);  
    ylabel('Depth (km)');
    rms_error=R2(orig_rtm_image,orig_rtm_image); 
    text(74.7453531598513, 256.815985130112,['Correlation coefficient = ' num2str(rms_error, '%4.2f')])
    text(123.908921933085, 218.340148698885,'Original migration image')
    ...
    for i = 1:7
        ...
        input_data = dlmread(['24shots_result_generate_' num2str(i) 'th_FWI_model' '/' 'train_outputs/export' export_num '.dat']);
        ...
        rms_error=R2(orig_rtm_image,rtm_image); 
        text(74.7453531598513, 256.815985130112,['Correlation coefficient = ' num2str(rms_error, '%4.2f')])
        if i == 1
            text(141.009293680297, 218.340148698885,[num2str(i) 'st migration image'])
        elseif i == 2
            text(141.009293680297, 218.340148698885,[num2str(i) 'nd migration image'])
        elseif i == 2
            text(141.009293680297, 218.340148698885,[num2str(i) 'rd migration image'])
        else
            text(141.009293680297, 218.340148698885,[num2str(i) 'th migration image'])
        end
        disp(['Correlation coefficient = ' num2str(rms_error, '%4.2f')])
