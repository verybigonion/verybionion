...
%     input_data = dlmread([num_shot 'shots' num_model 'samples' num_iter 'iterations_FWIuse_1st_iteration_FWI_model/real_outputs_using_vp_mig/real0.dat']);
    input_data = dlmread(['24shots_result_generate_3th_FWI_model' '/' 'real_outputs/real0.dat']);
elseif strcmp(mode,'train')
    %input train data:
    input_data = dlmread(['24shots_result_generate_0th_FWI_model' '/' 'train_outputs/export' export_num '.dat']);
end
%$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
...
xlim([1 nx]);ylim([1 nz]);
caxis([1.5 5.0]);  
rms_error=RMS(vp_true,CNN_output); 
text(142.8625, 205.529166666667,['RMS velocity error = ' num2str(rms_error*100, '%4.1f') '%'])
disp(['Initial RMS error = ' num2str(rms_error*100, '%4.1f') '%'])
...
vp = reshape(CNN_output,1,nz*nx);
name = '1st_iteration_FWI_model';
...
%%%%%%%%%%%%%%%%%%%%%
% plot FWI-derived migration result second
%%%%%%%%%%%%%%%%%%%%%
CNN_output_reflectivity = (CNN_output - vp_smooth)./vp_smooth;
%---------------------------------------------------------------
%------------------ Plot the image------------------------------
hfig1 = figure(4);
sh = 0.02;
...
caxis([-cvalue cvalue]);  
ylabel('Depth (km)');
rms_error=R2(reflectivity,reflectivity); 
text(142.8625, 205.529166666667,['RMS reflectivity error = ' num2str(rms_error*100, '%4.1f') '%'])
disp(['True RMS error = ' num2str(rms_error*100, '%4.1f') '%'])
text(-89.5208333333333, -55.5041666666666,'a)') %get from the code of figure
...
xlim([1 nx]);ylim([1 nz]);
caxis([-cvalue cvalue]);  
rms_error=R2(reflectivity,rtm_image); 
text(142.8625, 205.529166666667,['RMS reflectivity error = ' num2str(rms_error*100, '%4.1f') '%'])
disp(['Initial RMS error = ' num2str(rms_error*100, '%4.1f') '%'])
...
subaxis(3, 1, 3, 'sh', sh, 'sv', sv, 'padding', padding, 'margin', margin);
imagesc(CNN_output_reflectivity)
set(gca,'XAxisLocation','top');
set(gca, 'XTick', [ ])          
set(gca, 'YTick', [1 nx/6 nx/3])          
set(gca,'YTickLabel',{'0.00','1.44','2.88'})   
...
xlim([1 nx]);ylim([1 nz]);
caxis([-cvalue cvalue]);  
rms_error=R2(reflectivity,CNN_output_reflectivity); 
text(142.8625, 205.529166666667,['RMS reflectivity error = ' num2str(rms_error*100, '%4.1f') '%'])
disp(['Initial RMS error = ' num2str(rms_error*100, '%4.1f') '%'])
text(-87.9291666666666, 0.204166666666652,'c)') %get from the code of figure
...
plot(CNN_output_reflectivity(:,x_position),depth,'r','LineWidth',1.5);hold off;
set(gca,'YDir','reverse')
xlabel('Amplitude')
ylabel('Depth (km)')
set(gca,'ytick',[1 nz/4 nz/2 nz/4*3 nz]*dh) 
