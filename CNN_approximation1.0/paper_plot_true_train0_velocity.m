...
%     input_data = dlmread([num_shot 'shots' num_model 'samples' num_iter 'iterations_FWIuse_1st_iteration_FWI_model/real_outputs_using_vp_mig/real0.dat']);
    ...
    input_data = dlmread(['24shots_result_generate_' num2str(1) 'th_FWI_model' '/' 'train_outputs/export' export_num '.dat']);
    ...
    caxis([1.5 4.5]);  
    ylabel('Depth (km)');
    rms_error=RMS(orig_vp_true,vp_true); 
    text(166.659851301115, 256.815985130112,['RMS error = ' num2str(rms_error*100, '%4.1f') '%'])
    text(160.247211895911, 218.340148698885,['1st training model'])
    ...
    for i = 1:7
        subaxis(4, 2, i+1, 'sh', sh, 'sv', sv, 'padding', padding, 'margin', margin);
        input_data = dlmread(['24shots_result_generate_' num2str(i+1) 'th_FWI_model' '/' 'train_outputs/export' export_num '.dat']);
        ...
        text(166.659851301115, 256.815985130112,['RMS error = ' num2str(rms_error*100, '%4.1f') '%'])
        if i == 1
            text(160.247211895911, 218.340148698885,[num2str(i+1) 'nd training model'])
        elseif i == 2
            text(160.247211895911, 218.340148698885,[num2str(i+1) 'rd training model'])
        else
            text(160.247211895911, 218.340148698885,[num2str(i+1) 'th training model'])
        end
        disp(['RMS error = ' num2str(rms_error*100, '%4.1f') '%'])
