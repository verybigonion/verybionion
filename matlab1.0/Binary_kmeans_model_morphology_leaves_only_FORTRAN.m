...
vp_hor_mig=dlmread([dir '/' filename 'vp_hor.dat'])';
vp_nmo_mig=dlmread([dir '/' filename 'vp_nmo.dat'])';
...
vp_hor_true=dlmread([dir '/' filename2 'vp_hor.dat'])';
vp_nmo_true=dlmread([dir '/' filename2 'vp_nmo.dat'])';
...
segmented_vp_hor_layer = zeros(nz*nx,num_feature_layers);
segmented_vp_nmo_layer = zeros(nz*nx,num_feature_layers);
...
for i = 1:num_feature_layers
    ...
    segmented_vp_hor_layer(:,i) = vp_hor_mig.*sign(segmented_vp_layer(:,i));
    segmented_vp_nmo_layer(:,i) = vp_nmo_mig.*sign(segmented_vp_layer(:,i));
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
cmap_vp_hor = combine_kmeans_layers_boundary(segmented_vp_hor_layer);
disp(['vp_hor error=' num2str(sum(cmap_vp_hor-vp_hor_mig))])

cmap_vp_nmo = combine_kmeans_layers_boundary(segmented_vp_nmo_layer);
disp(['vp_nmo error=' num2str(sum(cmap_vp_nmo-vp_nmo_mig))])

if sum(cmap_vp-vp_mig) < 10 && sum(cmap_vs-vs_mig) < 30 && ...
        sum(cmap_rho-rho_mig) < 30 && sum(cmap_vp_hor-vp_hor_mig) < 10 ...
    && sum(cmap_vp_nmo-vp_nmo_mig) < 10
%     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% % %     
    ...
    hash_map_value_vp_hor = get_mean_std(segmented_vp_hor_layer);
    hash_map_value_vp_nmo = get_mean_std(segmented_vp_nmo_layer);
...
%     fclose(fid);
    ...
    vp_hor_new_set = zeros(nz*nx,num_new_model);
    k = 1;
    while k <= num_new_model
        hash_map_value_vp_hor_randn = normal_dist(hash_map_value_vp_hor(:,1),hash_map_value_vp_hor(:,2)*1,min_vp,max_vp); %std_factor == 1
        vp_hor_new = zeros(nz*nx,1);
        for i = 1:length(segmented_vp_hor_layer(1,:))
                vp_hor_new = vp_hor_new + sign(segmented_vp_hor_layer(:,i))*hash_map_value_vp_hor_randn(i);
        end
        r2_value(k,1) = R2(vp_hor_mig,vp_hor_new);
        r2_value(k,2) = R2(vp_hor_true,vp_hor_new);
        if r2_value(k,1) <min_r2 || r2_value(k,1) > max_r2
            continue; %skip this generated model
        else
            fid=fopen([velocity_dir '/' 'new_vp_hormodel' num2str(k) 'r2_sm' num2str(floor(r2_value(k,1)*100)) 'r2_tr' num2str(floor(r2_value(k,2)*100)) '.dat'],'wt');
            fprintf(fid,'%9.6f',vp_hor_new);
            fclose(fid);

            vp_hor_new_set(:,k) = vp_hor_new;
            show2d(vp_hor_new,nz,nx);caxis([1.5 2.5]);title([num2str(k) 'th model with R2 = ' num2str(r2_value(k,1))]);
            drawnow;
            pause(0.1);
            k = k + 1;
    ...
    vp_nmo_new_set = zeros(nz*nx,num_new_model);
    k = 1;
    while k <= num_new_model
        hash_map_value_vp_nmo_randn = normal_dist(hash_map_value_vp_nmo(:,1),hash_map_value_vp_nmo(:,2)*1,min_vp,max_vp); %std_factor == 1
        vp_nmo_new = zeros(nz*nx,1);
        for i = 1:length(segmented_vp_nmo_layer(1,:))
                vp_nmo_new = vp_nmo_new + sign(segmented_vp_nmo_layer(:,i))*hash_map_value_vp_nmo_randn(i);
        end
        r2_value(k,1) = R2(vp_nmo_mig,vp_nmo_new);
        r2_value(k,2) = R2(vp_nmo_true,vp_nmo_new);
        if r2_value(k,1) <min_r2 || r2_value(k,1) > max_r2
            continue; %skip this generated model
        else
            fid=fopen([velocity_dir '/' 'new_vp_nmomodel' num2str(k) 'r2_sm' num2str(floor(r2_value(k,1)*100)) 'r2_tr' num2str(floor(r2_value(k,2)*100)) '.dat'],'wt');
            fprintf(fid,'%9.6f',vp_nmo_new);
            fclose(fid);

            vp_nmo_new_set(:,k) = vp_nmo_new;
            show2d(vp_nmo_new,nz,nx);caxis([1.5 2.5]);title([num2str(k) 'th model with R2 = ' num2str(r2_value(k,1))]);
            drawnow;
            pause(0.1);
            k = k + 1;
