...
vel_dir='Marmousi/' #The velocity directory to store the predicted and smooth models
...
sh_filter_size=3
sh_water_depth=0 #Marmousi230z6500x
...
cd matlab1.0
  matlab_filename=${iteration}"th_mig_"  
  matlab -nodesktop -nosplash -noFigureWindows -r "prepare_CNN_real_test_data_FORTRAN;quit"
cd ..
...
matlab -nodesktop -nosplash -noFigureWindows -r "preprocess_rtm_real_FORTRAN;quit"
rm -r given*
rm -r out*
...
do
  ...
  #1. To do clustering and prepare training velocity models in fortran1.0 for nextth  step
    cd matlab1.0
      ...
      sh_filter_size=$sh_filter_size,\
      sh_water_depth=$sh_water_depth;\
      prepare_CNN_train_data_FORTRAN_vp_vs_rho_vphor_vpnmo;\
  ...
  # # 5. CNN training
    ...
    # 5.1 run vp_hor and vp_nmo first
    rm nohup*
    nohup python ML7_vp_horFWI.py --max_epochs $CNN_max_epochs \
    --store_weights_freq $store_weights_freq --CNN_num $CNN_last_epochs &

    python ML7_vp_nmoFWI.py --max_epochs $CNN_max_epochs \
    --store_weights_freq $store_weights_freq --CNN_num $CNN_last_epochs


  # 5.2 check the status of the four threads
    jobs -l > CNN_run.txt
    i=0
    while [ -s CNN_run.txt ]
    do
  ...
  # # 7. CNN prediction and export
    ...
    python ML7_vp_horFWI.py --mode real --CNN_num $CNN_last_epochs  #> 'temp.txt'
    python ML7_vp_horFWI.py --mode export --CNN_num $CNN_last_epochs   #> 'temp.txt'

    python ML7_vp_nmoFWI.py --mode real --CNN_num $CNN_last_epochs  #> 'temp.txt'
    python ML7_vp_nmoFWI.py --mode export --CNN_num $CNN_last_epochs   #> 'temp.txt'

    python ML7_vpFWI.py --mode real --CNN_num $CNN_last_epochs  #> 'temp.txt'
    python ML7_vpFWI.py --mode export --CNN_num $CNN_last_epochs   #> 'temp.txt'
    python ML7_vsFWI.py --mode real --CNN_num $CNN_last_epochs_vs  #> 'temp.txt'
    python ML7_vsFWI.py --mode export --CNN_num $CNN_last_epochs_vs   #> 'temp.txt'
    python ML7_rhoFWI.py --mode real --CNN_num $CNN_last_epochs  #> 'temp.txt'
    python ML7_rhoFWI.py --mode export --CNN_num $CNN_last_epochs  #> 'temp.txt'
  ...
  # 8. clean the directory for next iteration
    ...
    mv "CNN_weights_vp_nmo"$CNN_last_epochs ${iteration}${CNN_result_dir}
    mv "CNN_weights_vp_hor"$CNN_last_epochs ${iteration}${CNN_result_dir}
    rm -r CNN*
    rm train_dataset
    ...
    cp -r ${iteration}${CNN_result_dir}"/CNN_weights_vp_nmo"$CNN_last_epochs .
    cp -r ${iteration}${CNN_result_dir}"/CNN_weights_vp_hor"$CNN_last_epochs .
  ...
  # 9. create the CNN-predicted velocity for the next iteration
    ...
    input_vp_hor_filename=${iteration}${CNN_result_dir}"real_outputs/real0_vp_hor.dat";
    input_vp_nmo_filename=${iteration}${CNN_result_dir}"real_outputs/real0_vp_nmo.dat";
    ...
    output_vp_hor_filename="../"$matlab_dir$vel_dir${next_iter}"th_true_vp_hor"$postfix
    output_vp_nmo_filename="../"$matlab_dir$vel_dir${next_iter}"th_true_vp_nmo"$postfix
    ...
    sh_input_vp_hor_filename='$input_vp_hor_filename',sh_input_vp_nmo_filename='$input_vp_nmo_filename',\
    sh_output_vp_filename='$output_vp_filename',sh_output_vs_filename='$output_vs_filename',\
    sh_output_vp_hor_filename='$output_vp_hor_filename',sh_output_vp_nmo_filename='$output_vp_nmo_filename',\
